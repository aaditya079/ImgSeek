...
namespace ImgSeek
{
    ...
    public static class OcrScannerCore
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            {
                throw new UnauthorizedAccessException($"Could not scan folder contents: {ex.Message}. Please verify folder access permissions.", ex);
            }
            ...
            foreach (var imgPath in allImages)
            {
                ...
                {
                    // Skip single files that fail to load/OCR and report error
                    progress.Report(new ScanProgress
                    {
                        Current = currentCount,
                        Total = allImages.Count,
